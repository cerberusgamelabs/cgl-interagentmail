...
After the pending publisher exists, open the repository's **Actions > Publish to PyPI** workflow, choose **Run workflow**, keep tag `v1.4.0`, and approve the `pypi` environment deployment if prompted. The workflow builds and tests in a job without publishing credentials, then passes only the distributions to the OIDC-enabled publishing job.
...
```powershell
pipx install --force .\dist\cgl_interagentmail-1.4.0-py3-none-any.whl
iam --help
interagentmail --help
iam-mcp --help
```
...
```powershell
python -m twine upload --repository testpypi dist\cgl_interagentmail-1.4.0*
```
...
The normal release path is to push the approved version commit and its matching
`v1.4.0` tag. The tag starts `Publish to PyPI`, which tests and builds the package,
checks package contents, and publishes through the existing PyPI trusted publisher.
After publication, it creates the GitHub release with the wheel, source archive,
checksums, and changelog notes. If the `pypi` environment requires approval, a
maintainer must approve that deployment. Do not bypass the environment gate.

The manual commands below are alternatives, not additional steps after a successful
workflow publication. Never upload the same version twice.

PyPI releases are immutable. Confirm the version, metadata, README rendering, license, and checksums before uploading:

```powershell
python -m twine upload dist\cgl_interagentmail-1.4.0-py3-none-any.whl dist\cgl_interagentmail-1.4.0.tar.gz
```
